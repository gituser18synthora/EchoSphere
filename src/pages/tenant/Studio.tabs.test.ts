/* Studio tab registry: tabs carrying permission codes are shown only when the
   ...
   while Intents, APIs, Channels, Analytics and Publish require management
   permissions. (The menu is a UI affordance; the tab APIs enforce the same
...
describe("Studio tab permission gating", () => {
  it("shows every tab when the session holds all permissions (admin)", () => {
    const ids = visibleStudioTabs(() => true).map((t) => t.id);
    expect(ids).toEqual([
      "overview", "knowledge", "prompts", "voice", "intents", "apis",
      "workflows", "channels", "testing", "analytics", "publish",
    ]);
  });

  it("shows exactly the six allowed sections to a tenant user", () => {
    const ids = visibleStudioTabs((c) => TENANT_USER_PERMS.includes(c)).map((t) => t.id);
    expect(ids).toEqual([
