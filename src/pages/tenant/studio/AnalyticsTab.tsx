...
export default function AnalyticsTab({ bot }: { bot: VoiceBot }) {
  ...
  // Server-enforced: the analytics response carries no costSeries without
  // costs.view; this hides the empty chart.
  ...
  return (
    <div className="col gap-16">
      <div className="grid grid-4">
        {a.data.kpis.filter((k) => showCosts || !isCostLabel(k.label)).slice(0, 4).map((k, i) => (
          <KpiCard
            key={k.label}
            label={k.label}
            value={k.value}
      ...
      <div className={showCosts ? "grid grid-2" : "grid"}>
        {showCosts && (
          <ChartCard
