...
class TestAdminChosenPasswordReset:
    """Edit Tenant → reset the tenant admin's password to an admin-chosen value
    (POST /users/{id}/reset-password with newPassword/confirmPassword)."""

    @pytest.fixture(scope="class")
    def target(self, client, super_admin):
        email = f"pwreset-{_SUFFIX}@example.com"
        created = _data(client.post(f"{API}/users", headers=super_admin, json={
            "name": "Reset Target", "email": email, "roleCode": "tenant_admin",
            "tenantId": "tn-001", "password": "Original@2026x",
        }))
        _created.append(("users", created["id"]))
        return {"id": created["id"], "email": email, "password": "Original@2026x"}
    ...
    def test_mismatch_rejected(self, client, super_admin, target):
        response = self._reset(client, super_admin, target["id"],
                               "Chosen@2026pw", "Different@2026pw")
        assert response.status_code == 422
        assert "do not match" in response.json()["message"]
        # password unchanged
        assert self._login(client, target["email"], target["password"]).status_code == 200

    def test_weak_password_rejected(self, client, super_admin, target):
        for weak in ("short1A", "alllowercase1", "ALLUPPERCASE1", "NoDigitsHere"):
            response = self._reset(client, super_admin, target["id"], weak, weak)
            assert response.status_code == 422, weak
            assert "Password must contain" in response.json()["message"]
        assert self._login(client, target["email"], target["password"]).status_code == 200

    def test_empty_fields_do_not_issue_temp_password(self, client, super_admin, target):
        """Explicit empty strings must not silently fall back to a temporary
        password — the account's password stays untouched."""
        response = self._reset(client, super_admin, target["id"], "", "")
        assert response.status_code == 422
        assert "temporaryPassword" not in (response.json().get("data") or {})
        assert self._login(client, target["email"], target["password"]).status_code == 200

    def test_unauthorized_user_rejected(self, client, tenant_user, target):
        response = self._reset(client, tenant_user, target["id"],
                               "Chosen@2026pw", "Chosen@2026pw")
        assert response.status_code == 403
        assert self._login(client, target["email"], target["password"]).status_code == 200

    def test_successful_reset_and_session_invalidation(self, client, super_admin, target):
        from datetime import datetime, timedelta, timezone
        ...
        # An existing session for the target, issued before the reset.
        settings = get_settings()
        old_token = pyjwt.encode(
            {"sub": target["id"], "role": "tenant_admin", "tenant_id": "tn-001",
             "iat": datetime.now(timezone.utc) - timedelta(seconds=60),
             "exp": datetime.now(timezone.utc) + timedelta(hours=1)},
            settings.jwt_secret, algorithm=settings.jwt_algorithm,
        )
        ...
        result = _data(self._reset(client, super_admin, target["id"],
                                   "Chosen@2026pw", "Chosen@2026pw"))
        assert result["reset"] is True and result["sessionsInvalidated"] is True
        # The chosen password is never echoed back.
        assert "temporaryPassword" not in result

        # Old password stops working; the chosen one signs in without rotation.
        assert self._login(client, target["email"], target["password"]).status_code == 401
        login = self._login(client, target["email"], "Chosen@2026pw")
        assert login.status_code == 200
        target["password"] = "Chosen@2026pw"

        # The pre-reset session is signed out.
        assert client.get(f"{API}/auth/me",
                          headers={"Authorization": f"Bearer {old_token}"}).status_code == 401

    def test_audit_entry_without_plaintext(self, client, target):
        from sqlalchemy import select
        ...
        try:
            row = session.execute(
                select(AuditLog).where(
                    AuditLog.action == "Reset user password",
                    AuditLog.entity_id == target["id"],
                ).order_by(AuditLog.created_at.desc()).limit(1)
            ).scalar_one_or_none()
            assert row is not None
            assert (row.new_value or {}).get("method") == "admin-set"
            blob = str(row.previous_value) + str(row.new_value)
            assert "Chosen@2026pw" not in blob and "Original@2026x" not in blob
        finally:
            session.close()

    def test_self_reset_blocked(self, client, super_admin):
        from sqlalchemy import select
        ...
        try:
            ...
            session.close()
        response = self._reset(client, super_admin, me.id, "Chosen@2026pw", "Chosen@2026pw")
        assert response.status_code == 400
