...
**Status:** verified on 2026-07-28 against the repository, current `.env`,
...
this document wins — see [PDF vs Current Implementation](#pdf-vs-current-implementation)
...
is a short-lived bearer credential for one active call pipeline. The historical API also mounts
the same handler at `/api/v1/telephony/webhook/vaani` on port 9001, but that is
not the Vaani integration endpoint; Vaani must use the port-9011 URL above.
...
Error bodies are JSON: `{"success": false, "message": "<human-readable reason>"}`.
...
| `404` | `botId` unknown, archived, unpublished, **or belongs to another tenant** (existence is deliberately not revealed) | `Bot not found.` |
...
| `500` | Unexpected server error | sanitized generic message |
...
| Caller silence monitor | 48 s with the current 12 s setting. It logs recurring idle warnings but `cancel_on_idle_timeout=False`, so silence alone does **not** disconnect the call |
...
sequenceDiagram
    ...
    GW-->>D: close 4400
    note over D: recovery is ALWAYS: new webhook → new URL
```
...
| `DEFAULT_SILENCE_TIMEOUT` | `12` | Base for the non-terminating 48 s pipeline idle monitor |
...
WebSocket*, eDAS) predates the deployed system. Item-by-item:

| Item | PDF says | EchoSphere implements | Match? | Risk / who changes |
|---|---|---|---|---|
| Connection bootstrap | Static WS endpoint (`wss://<voicebot-url>/stream/voicebot`), no webhook | **Signed HTTP webhook per call** → per-call WS URL | **No** | **Vaani must adopt the webhook flow.** A static endpoint cannot identify tenant/bot/call and has no auth. |
| Authentication | "API token must be kept confidential" (mechanism unspecified) | HMAC-SHA256 signature + timestamp + single-use replay protection on the webhook; WS authenticated by the single-use session URL | **No (stronger)** | Vaani implements signature generation (§2.2). |
| Transport security | Always `wss://` (port 443) | `ws://` on the private LAN deployment at `192.168.60.123:9011`; `wss://` possible behind a TLS proxy later | Partial | Acceptable on the private network; revisit for any public exposure. |
| Reconnect | "Reconnect with exponential backoff" | A pre-pipeline `4400`/`4429` may retry the still-issued URL; an established call cannot resume after a drop and needs a fresh webhook | Partial | **Vaani must distinguish setup retry from call resume.** |
| `connected` event | Event section shows Vaani→VoiceBOT, while the architecture section says VoiceBOT responds with it | Vaani may send it; EchoSphere accepts/ignores it and sends no acknowledgement | Partial / PDF inconsistent | Vaani must not wait for a `connected` response. |
| `start` event | `streamSid` + `start.track` + `start.mediaFormat` (audio/lin, 8000, 1) | Required; `streamSid` (top level or inside `start`) and media rate/channels validated; `track` and encoding ignored | Yes, with validation caveat | Send all PDF fields even though two are not validated. |
| Inbound `media` | `chunk` sequential string, `timestamp` epoch, `payload` base64 | Same; non-increasing numeric chunks and foreign non-empty `streamSid` dropped; base64 text over 140,000 chars dropped | Yes (+ robustness) | Keep decoded audio ≤100,000 bytes. |
| Inbound `stop` | Terminates session | Same — ends the call even if the socket stays open | Yes | None. |
| Outbound `media` | `track`, `chunk`, `timestamp`, `payload` | Identical shape; `track` is always `"inbound"`, `timestamp` is epoch seconds | Yes | None. |
| `clear` | `{"clear":{"reason":"interrupt"}}` — flush playback | Identical, sent on barge-in | Yes | Vaani must actually flush buffered audio promptly. |
| `transfer` | `reason` + optional `transfer_queue`, `agent_id` | Identical shape; `agent_id` never emitted today; sent **after** the announcement finishes | Yes | Confirm Vaani's queue identifiers (open question 1). |
| Outbound `stop` | Either side may send | Same; EchoSphere guarantees exactly one, with nothing after it | Yes | None. |
| Chunk rules | 3.2 KB–100 KB, multiple of 320, "bidirectional" | Outbound targets ≥3,200 bytes, is aligned to 320, and effectively caps at 99,840; final outbound may be shorter. Inbound accepts real-time 320-byte frames and does not enforce the PDF minimum/alignment | Partial | Vaani may send paced 320-byte frames and must accept short final outbound chunks. The PDF's 3.2 KB is 200 ms, not 100 ms. |
| Audio format | 8 kHz, 16-bit PCM, mono, base64 | Identical (raw PCM, little-endian, no WAV header) | Yes | None. |
| DTMF / `mark` events | Not defined | Not processed; unknown events ignored safely | n/a | If DTMF is needed, it's a new feature (open question 3). |
| Error event | Not defined | No application `error` event; fatal setup errors are WebSocket close codes | n/a | Log close/HTTP failures with `callId` + session id. |
| Error handling | "Log and replay failed messages" | Numeric duplicate/old media chunks are discarded; an exact webhook replay is rejected, and a freshly signed retry is not idempotent | Partial | Do not replay media or blindly retry setup. Reconcile/close the old attempt before creating a replacement session. |
| Session identifier in webhook response | No webhook contract | Response contains only `url`; session id is its final path segment | New | Extract for logs only; use the returned URL unchanged. |
| Silence/max duration | Not specified | 48 s idle monitor does not cancel; 900 s transport timeout currently ends the call before the 3,600 s timer | New | Plan call duration around the effective 900 s ceiling unless EchoSphere changes configuration/code. |
...
resampled), `--no-rewrite` (connect to the returned public URL as-is instead
of rewriting the host to the `--base` host for local runs).
