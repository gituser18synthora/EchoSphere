"""Zepto MDND four-fact collector — slot semantics (extension data + rules).

Canonical facts (customer_called, reached_location, delivery_handoff,
cx_support_called) are checkpointed alongside the legacy ticket/report
fields. Only definitions opting into ``semanticSlots: "mdnd_v1"`` use this.
Every Zepto literal (slot names, canonical values, the verification-hub and
narrative-ask node ids, Hindi/English summary wording) lives HERE, not in the
engine.
"""
...
# Node roles the engine asks the provider about. A definition may name them
# explicitly (``semanticRole: "summary_hub" | "narrative"`` on the node);
# the ids below are the defaults of the deployed Zepto MDND definitions.
DEFAULT_SUMMARY_HUB_NODE = "n_hub_verify"
DEFAULT_NARRATIVE_VARIABLE = "m_issue_description"


def is_summary_hub(node) -> bool:
    config = (node or {}).get("config") or {}
    role = config.get("semanticRole")
    if role:
        return role == "summary_hub"
    return (node or {}).get("id") == DEFAULT_SUMMARY_HUB_NODE


def is_narrative_ask(node, variable) -> bool:
    config = (node or {}).get("config") or {}
    role = config.get("semanticRole")
    if role:
        return role == "narrative"
    return variable == DEFAULT_NARRATIVE_VARIABLE
