...
_DEVANAGARI_FORMS = {
    "male": (
        # Participles agree with the SUBJECT, and a first-person sentence
        # can still carry passive/object participles that must not move:
        # "आपके द्वारा दी गई जानकारी … कर लेता हूँ" (जानकारी is feminine — cv_b80077e273d8
        # spoke "दी गया जानकारी"). Only the speaker's own predicate is
        # adapted: the form right before हूँ/हूं, or the compound "रही थी".
        (r"(?<!\w)रही(?=\s*(?:हूँ|हूं)(?!\w))", "रहा"),
        (r"(?<!\w)चुकी(?=\s*(?:हूँ|हूं)(?!\w))", "चुका"),
        (r"(?<!\w)(?:गई|गयी)(?=\s*(?:हूँ|हूं)(?!\w))", "गया"),
        (r"(?<!\w)बैठी(?=\s*(?:हूँ|हूं)(?!\w))", "बैठा"),
        (r"(?<!\w)खड़ी(?=\s*(?:हूँ|हूं)(?!\w))", "खड़ा"),
        (r"(?<!\w)(रही|चुकी|बैठी|खड़ी)\s+थी(?!\w)",
         lambda m: {"रही": "रहा", "चुकी": "चुका", "बैठी": "बैठा", "खड़ी": "खड़ा"}[m.group(1)] + " था"),
        (r"(?<!\w)(?:गई|गयी)\s+थी(?!\w)", "गया था"),
        # Habitual/modal forms (करती, सकती, जाती …) are adapted ONLY as the
        # speaker's own predicate — right before हूँ/हूं. A bare form elsewhere
        # in a first-person sentence agrees with its own noun: "थोड़ा बता देता
        # हूँ — rider से onboarding fee ली जाती है, जो … अलग हो सकती है" must
        # keep जाती/सकती (cv_10dd1b13a5e1 spoke "fee ली जाता है").
        (r"(?<!\w)करती(?=\s*(?:हूँ|हूं)(?!\w))", "करता"),
        (r"(?<!\w)सकती(?=\s*(?:हूँ|हूं)(?!\w))", "सकता"),
        (r"(?<!\w)वाली(?=\s*(?:हूँ|हूं)(?!\w))", "वाला"),
        (r"(?<!\w)देती(?=\s*(?:हूँ|हूं)(?!\w))", "देता"),
        (r"(?<!\w)लेती(?=\s*(?:हूँ|हूं)(?!\w))", "लेता"),
        (r"(?<!\w)जाती(?=\s*(?:हूँ|हूं)(?!\w))", "जाता"),
        (r"(?<!\w)पाती(?=\s*(?:हूँ|हूं)(?!\w))", "पाता"),
        (r"(?<!\w)जानती(?=\s*(?:हूँ|हूं)(?!\w))", "जानता"),
        (r"(?<!\w)लगती(?=\s*(?:हूँ|हूं)(?!\w))", "लगता"),
        (r"(?<!\w)बोलती(?=\s*(?:हूँ|हूं)(?!\w))", "बोलता"),
        (r"(?<!\w)बताती(?=\s*(?:हूँ|हूं)(?!\w))", "बताता"),
        (r"(?<!\w)चाहती(?=\s*(?:हूँ|हूं)(?!\w))", "चाहता"),
        (r"(?<!\w)समझती(?=\s*(?:हूँ|हूं)(?!\w))", "समझता"),
        (r"(?<!\w)रखती(?=\s*(?:हूँ|हूं)(?!\w))", "रखता"),
        (r"([\u0900-\u097f]+)ूँगी(?!\w)", r"\1ूँगा"),
    ),
    "female": (
        (r"(?<!\w)रहा(?=\s*(?:हूँ|हूं)(?!\w))", "रही"),
        (r"(?<!\w)चुका(?=\s*(?:हूँ|हूं)(?!\w))", "चुकी"),
        (r"(?<!\w)गया(?=\s*(?:हूँ|हूं)(?!\w))", "गई"),
        (r"(?<!\w)बैठा(?=\s*(?:हूँ|हूं)(?!\w))", "बैठी"),
        (r"(?<!\w)खड़ा(?=\s*(?:हूँ|हूं)(?!\w))", "खड़ी"),
        (r"(?<!\w)(रहा|चुका|गया|बैठा|खड़ा)\s+था(?!\w)",
         lambda m: {"रहा": "रही", "चुका": "चुकी", "गया": "गई", "बैठा": "बैठी", "खड़ा": "खड़ी"}[m.group(1)] + " थी"),
        (r"(?<!\w)करता(?=\s*(?:हूँ|हूं)(?!\w))", "करती"),
        (r"(?<!\w)सकता(?=\s*(?:हूँ|हूं)(?!\w))", "सकती"),
        (r"(?<!\w)वाला(?=\s*(?:हूँ|हूं)(?!\w))", "वाली"),
        (r"(?<!\w)देता(?=\s*(?:हूँ|हूं)(?!\w))", "देती"),
        (r"(?<!\w)लेता(?=\s*(?:हूँ|हूं)(?!\w))", "लेती"),
        (r"(?<!\w)जाता(?=\s*(?:हूँ|हूं)(?!\w))", "जाती"),
        (r"(?<!\w)पाता(?=\s*(?:हूँ|हूं)(?!\w))", "पाती"),
        (r"(?<!\w)जानता(?=\s*(?:हूँ|हूं)(?!\w))", "जानती"),
        (r"(?<!\w)लगता(?=\s*(?:हूँ|हूं)(?!\w))", "लगती"),
        (r"(?<!\w)बोलता(?=\s*(?:हूँ|हूं)(?!\w))", "बोलती"),
        (r"(?<!\w)बताता(?=\s*(?:हूँ|हूं)(?!\w))", "बताती"),
        (r"(?<!\w)चाहता(?=\s*(?:हूँ|हूं)(?!\w))", "चाहती"),
        (r"(?<!\w)समझता(?=\s*(?:हूँ|हूं)(?!\w))", "समझती"),
        (r"(?<!\w)रखता(?=\s*(?:हूँ|हूं)(?!\w))", "रखती"),
...
def _adapt_first_person_part(part: str, gender: str) -> str:
    ...
    for source, replacement in _ROMAN_FORMS[gender]:
        # Only the speaker's own predicate ("main samajh sakti hoon"), never a
        # form that agrees with another noun ("fee li jaati hai").
        part = re.sub(
            rf"(?<![A-Za-z]){source}(?=\s*(?:hoon|hun|hu|hoo)(?![A-Za-z]))",
