...
try:
    from shared.knowledge.parsing.cleaners import preprocess_for_embedding
    HAS_CLEANERS = True
except ImportError:
    HAS_CLEANERS = False
...
try:
    from shared.knowledge.chunking.chunker import create_chunks, CHUNK_CONFIGS
    HAS_CHUNKER = True
except ImportError:
    HAS_CHUNKER = False
...
from shared.knowledge.parsing.ocr import (
    OcrBudget,
    analyze_page_images,
    ocr_enabled,
    run_page_ocr,
...
def _try_layout_aware_chunking(file_path_obj, ext: str, source_label: str) -> Optional[List[Dict]]:
    ...
    """
    try:
        from shared.knowledge.parsing.doc_classifier import detect_document_type
        from shared.knowledge.chunking.structured_chunker import build_structured_chunks, build_embedding_text
