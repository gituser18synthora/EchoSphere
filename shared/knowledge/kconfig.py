"""Configuration shim for the KMRAG modules ported into shared.knowledge.
...
`from shared.knowledge.kconfig import settings` with minimal diff.
...
Value sources:
  - Attributes that have an equivalent in EchoSphere's backend/config.py
    ...
    from shared.config.get_settings() so there is a single source of truth.
