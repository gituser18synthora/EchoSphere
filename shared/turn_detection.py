...
TURN_DETECTION_FIELDS: tuple[dict[str, Any], ...] = (
    ...
    # before the first "can you hear me?" prompt, the interval between retries
    # and how many prompts precede the polite close. Whole numbers; the close
    # wording and the post-hold grace stay platform settings.
    ...
    {"group": "turn_detection", "key": "speaker_consistency_mode", "section": "background_speech", "label": "Caller voice consistency", "description": "Compare each accepted caller segment with the voice of turns the call already vouched for. 0 = off, 1 = shadow (evidence only, never changes behaviour), 2 = enforce (reserved; no behaviour in the current build).", "input": "number", "valueType": "number", "unit": "mode", "min": 0.0, "max": 2.0, "step": 1.0, "default": {"browser": 0.0, "telephony": 0.0}, "recommended": {"browser": 0.0, "telephony": 0.0}},
    ...
    {"group": "noise_gate", "key": "echo_reference_mode", "section": "echo_protection", "label": "Self-echo reference mode", "description": "Compare caller audio against the bot's own outgoing audio (telephony only). 0 = off, 1 = shadow (evidence only, never suppresses), 2 = enforce (suppress frames that are the bot's own echo).", "input": "number", "valueType": "number", "unit": "mode", "min": 0.0, "max": 2.0, "step": 1.0, "default": {"browser": 0.0, "telephony": 0.0}, "recommended": {"browser": 0.0, "telephony": 0.0}},
    {"group": "noise_gate", "key": "echo_reference_max_lag_ms", "section": "echo_protection", "label": "Self-echo lag search span", "description": "Longest echo delay the self-echo reference searches for (telephony only).", "input": "number", "valueType": "number", "unit": "ms", "min": 100.0, "max": 1500.0, "step": 50.0, "default": {"browser": 700.0, "telephony": 700.0}, "recommended": {"browser": 700.0, "telephony": 700.0}},
)
