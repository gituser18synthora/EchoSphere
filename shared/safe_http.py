...
@dataclass
class JsonResponse:
    """One guarded JSON request-response for tool execution.

    Unlike SafeResponse (a truncated PREVIEW for the Studio test button),
    this carries the parsed body — tool results feed workflows and prompts,
    so the runtime needs the real object, still size-capped at read time.
    """
...
def fetch_json(
    *,
    ...
    """One guarded request returning the parsed JSON body. Never raises for
    network/parse errors — tools must degrade, not kill a call."""
    ...
    try:
        ...
        return JsonResponse(status_code=0, ok=False, latency_ms=0, error=str(exc))
    ...
    try:
        ...
        return JsonResponse(
            status_code=504, ok=False,
            latency_ms=round((time.monotonic() - started) * 1000),
            error=f"Upstream timeout after {timeout_ms}ms.",
        )
    except httpx.HTTPError as exc:
        return JsonResponse(
            status_code=502, ok=False,
