...
SUITES = {
    "MDND": (STATE["BOT_MDND"], [
        ...
        # to verification (a guard's name is never a required question).
         ...
         [("mdnd wala issue hai", [(TR, "n_ask_issue_desc")]),
          ("haan maine call kiya tha aur uske location par bhi gaya tha, "
           ...
           [(SL, '"m_reached_location": "yes (reached the location)"'),
            ...
            (TR, "n_hub_verify"), (TN, "n_ask_reached_called"),
            (TN, "n_ask_guard_name_known"), (R, "सही है"),
            # The summary legitimately SAYS the answers; only the question
            # wordings count as a re-ask.
            (RN, "किसको सौंपा"), (RN, "delivery से पहले"), (RN, "कोई call आया था"),
            (RN, "नाम"), (SM, '"call_cx": "yes"')]),
          ("sahi hai",
        ...
        ("MDND 03 partial answer: only the missing questions are asked",
         [("mdnd ka issue hai", [(TR, "n_ask_issue_desc")]),
          ...
          ("haan call kiya tha",
           # Guard handover: no guard-name question — CX is the only open fact.
           [(SL, '"m_called_customer": "yes (called the customer)"'),
            (TR, "n_ask_cx"), (TN, "n_ask_guard_name_known"),
            (RN, "सौंपा"), (RN, "नाम"), (R, "cx support")]),
        ...
        ("MDND 04 combined question + correction at the verification step",
         [("mdnd issue hai", [(TR, "n_ask_issue_desc")]),
          ...
          ("actually order maine guard ko diya tha customer ko nahi",
           # Only the recipient changes; reached/called/CX are NOT re-asked
           # and the corrected summary is confirmed again at once.
           [(SL, '"m_handover_recipient": "guard / security"'),
            (SL, '"m_called_customer": "yes (called the customer)"'),
            (SL, '"m_cx_support_call": "no (no cx support call)"'),
            (TN, "n_ask_reached_called"), (RN, "कोई call आया था"),
            (RN, "location पर पहुंचे"), (RN, "delivery से पहले"),
            (TN, "n_ask_guard_name_known"), (RN, "नाम"),
            (TR, "n_hub_verify"), (R, "सही है"),
        ...
        ("MDND 09 no context partial: asks only missing date, then CX",
         [("mdnd issue hai", None, [(TR, "n_ask_issue_desc")],
           {"contextSource": "none"}),
        ...
        ("MDND 18 cv_a00399bcc37b narrative: nothing re-asked",
         [("haan bol raha hoon", [(TR, "n_ask_issue_desc"), (RN, "onboarding")]),
          ...
          ("security guard ko diya tha",
           [(SL, '"m_handover_recipient": "guard / security"'),
            (TR, "n_hub_verify"), (R, "सही है"), (TN, "n_ask_guard_name_known"),
            (SM, '"hand_over_to": "security_guard"')]),
          ("haan ab sahi hai", [(TR, "n_api"), (TN, "n_ask_other")]),
          ]),
        ...
        # A guard handover is one of the four facts — the guard's NAME is never
        # asked (volunteered names are still stored).
        ("MDND 20 guard handover (Devanagari) -> no name question, CX next",
         [("haan bol raha hoon", [(TR, "n_ask_issue_desc")]),
          ("मैंने कस्टमर को कॉल किया, लोकेशन पर पहुँच के कॉल किया। कस्टमर ने बोला "
           ...
           [(SL, '"m_handover_recipient": "guard / security"'),
            (TR, "n_ask_cx"), (TN, "n_ask_guard_name_known"), (RN, "नाम"),
            (R, "cx support")]),
          ("हाँ, CX support से कॉल आया था।",
           [(SL, '"m_cx_support_call": "yes (received cx support call)"'),
            (TR, "n_hub_verify"), (R, "सही है")]),
          ]),
        ("MDND 21 guard handover (Hinglish) -> no name question, CX next",
         [("haan bol raha hoon", [(TR, "n_ask_issue_desc")]),
          ("location par pahuncha, call kiya aur guard ko de diya",
           [(SL, '"m_handover_recipient": "guard / security"'),
            (TR, "n_ask_cx"), (TN, "n_ask_guard_name_known"), (RN, "नाम")]),
          ("nahi, koi call nahi aaya",
           [(SL, '"m_cx_support_call": "no (no cx support call)"'),
            (TR, "n_hub_verify"), (R, "सही है")]),
          ]),
